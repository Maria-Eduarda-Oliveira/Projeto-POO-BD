...
public class JogadoresDAO extends ConnectionDAO{
    ...
    public boolean insertJogadores(Jogadores jogadores) { //função para inserir jogadores na tabela
        connectToDB();
        String sql = "INSERT INTO jogadores (CPF, nome, altura, peso, n_da_camisa) values(?,?,?,?,?)";
        try {
    ...
    public ArrayList<Jogadores> selectJogadores() { // seleciona os jogadores com mais de 1.70
        ...
        try {
            ...
            while (rs.next()) {
                Jogadores jogadoresAux = new Jogadores(rs.getInt("cpf"), rs.getString("nome"), rs.getInt("altura"), rs.getInt("peso"), rs.getInt("n_da_camisa"));
